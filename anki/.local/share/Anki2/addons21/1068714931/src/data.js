    ...
    if (settings.graphMode === 'default') {
        ...
        for (let i0 = 0; i0 < notes.length; i0++) {
            ...
            for (let i1 = 0; i1 < notes.length; i1++) {
                if ( i1 <= i0 || notes[i1][0] === notes[i0][0]) {
                    continue;
                }
                if (searchResults.length && (!searchResults.includes(notes[i1][0]) || !searchResults.includes(nid0))) {
                    continue;
                }
                ...
                // collect points
                //
                let idDiff = Math.abs(nid0 - nid1) / 1000;
                ...
                if (tags1.length) {
                    for (let t0 of tags0 || []) {
                        ...
                        let tc = tagCounts[t0];
                        if (tc &&  tags1.includes(t0) && tc <= settings.tagBoundary) {
